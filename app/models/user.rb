...
class User < ActiveRecord::Base
  attr_accessible :username, :email, :password, :password_confirmation, :captcha, :captcha_key, :is_admin, :remember_token
  ...
  # validations
  validates :username, :uniqueness => {:message => '用户名已经存在'}, :presence => { :message => '请填写用户名'}, :length => {:to_long => "用户名不能超过%{count}个字符", :maximum => 64,  :minimum   => 4, :too_short => "用户名太短了喔"}
